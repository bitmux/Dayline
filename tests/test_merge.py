...
from_standing = merge.from_standing
standing_key = merge.standing_key

OPEN_AT = datetime(2026, 9, 2, 10, 42, tzinfo=TZ)


def snap(state: str, since: datetime | None = OPEN_AT, name: str = "Garage Door"):
    return {"state": state, "since": since.isoformat() if since else None, "name": name}


def test_a_standing_row_appears_while_the_state_matches():
    rules = [{"entity_id": "cover.garage", "state": ["open"], "phrase": "Garage is open"}]
    out = from_standing(rules, {"cover.garage": snap("open")}, DAY_START)
    assert len(out) == 1
    assert out[0]["title"] == "Garage is open"
    assert out[0]["kind"] == "standing"
    # Sticky and high together are what keep it on the card: the budget cannot
    # collapse it and its time passing cannot strike it through.
    assert out[0]["sticky"] is True
    assert out[0]["priority"] == "high"
    assert out[0]["start"] == OPEN_AT.isoformat()


def test_it_leaves_when_the_state_does():
    rules = [{"entity_id": "cover.garage", "state": ["open"], "phrase": "Garage is open"}]
    assert from_standing(rules, {"cover.garage": snap("closed")}, DAY_START) == []


def test_any_of_several_states_counts():
    """The interesting states rarely have exactly one name."""
    rules = [{"entity_id": "cover.garage", "state": ["open", "opening"], "phrase": "Garage is open"}]
    assert len(from_standing(rules, {"cover.garage": snap("opening")}, DAY_START)) == 1


def test_states_written_as_one_string_still_work():
    """Tolerated so a rule hand-written in YAML, or saved before this took a
    list, does not silently stop matching."""
    rules = [{"entity_id": "cover.garage", "state": "open, opening", "phrase": "Open"}]
    assert len(from_standing(rules, {"cover.garage": snap("opening")}, DAY_START)) == 1


def test_something_true_since_yesterday_starts_at_the_top_of_today():
    """Otherwise it sorts above breakfast and reads as this morning's news."""
    rules = [{"entity_id": "cover.garage", "state": ["open"], "phrase": "Garage is open"}]
    tuesday = datetime(2026, 8, 30, 22, 0, tzinfo=TZ)
    out = from_standing(rules, {"cover.garage": snap("open", tuesday)}, DAY_START)
    assert out[0]["start"] == DAY_START.isoformat()


def test_the_button_always_runs_a_script():
    """A script is the only place a decision can live. Calling the cover
    directly would work and would skip the camera check."""
    rules = [
        {
            "entity_id": "cover.garage",
            "state": ["open"],
            "phrase": "Garage is open",
            "script": "script.close_garage_safely",
            "button": "Close it",
        }
    ]
    out = from_standing(rules, {"cover.garage": snap("open")}, DAY_START)
...
def test_no_script_means_no_button_but_still_a_row():
    rules = [{"entity_id": "cover.garage", "state": ["open"], "phrase": "Garage is open"}]
    out = from_standing(rules, {"cover.garage": snap("open")}, DAY_START)
    assert "action" not in out[0]


def test_a_template_rule_is_keyed_by_position_and_reads_on_or_off():
    rules = [{"template": "{{ 1 > 0 }}", "phrase": "Something is up"}]
    key = standing_key(rules[0], 0)
    assert key == "template:0"
    assert len(from_standing(rules, {key: snap("on")}, DAY_START)) == 1
    assert from_standing(rules, {key: snap("off")}, DAY_START) == []


def test_a_template_rule_may_still_name_an_entity_for_its_id():
    """Named so tapping the row opens the right more-info dialog. The condition
    is still the template's to decide."""
    rules = [{"template": "{{ true }}", "entity_id": "cover.garage", "phrase": "Up"}]
    out = from_standing(rules, {"cover.garage": snap("on")}, DAY_START)
    assert out[0]["entity_id"] == "cover.garage"


def test_one_entity_yields_one_row_whichever_state_matched():
    """A cover going open -> opening must update a row, not grow a second."""
    rules = [{"entity_id": "cover.garage", "state": ["open", "opening"], "phrase": "Open"}]
    a = from_standing(rules, {"cover.garage": snap("open")}, DAY_START)
    b = from_standing(rules, {"cover.garage": snap("opening")}, DAY_START)
    assert a[0]["id"] == b[0]["id"]


def test_an_unknown_entity_is_skipped_rather_than_guessed_at():
    rules = [{"entity_id": "cover.nope", "state": ["open"], "phrase": "Open"}]
    assert from_standing(rules, {}, DAY_START) == []


def test_a_standing_row_counts_as_left_today():
    """It is genuinely something still to deal with."""
    rules = [{"entity_id": "cover.garage", "state": ["open"], "phrase": "Garage is open"}]
    out = from_standing(rules, {"cover.garage": snap("open")}, DAY_START)
    assert remaining_count(out, NOW) == 1
