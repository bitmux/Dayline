...
def standing_key(rule: dict[str, Any], index: int) -> str:
    """What a standing rule is looked up by in the snapshot.

    An entity rule is keyed by its entity, so one entity yields one row however
    many states it matches. A template rule has no entity to be keyed by, so it
    is keyed by position — the coordinator and this module derive it the same
    way, from the same list, which is why the definition lives in one place.
    """
    return str(rule.get("entity_id") or f"template:{index}")
...
def from_standing(
    ...
    The counterpart to the "what just happened" lines, and the opposite trade.
    Those are event-driven and expire: the house did something, here is a
    sentence, gone in five minutes. These are state-driven and never expire — a
    door that opened at 10:42 stops being news long before it stops being a
    problem, and a card whose job is "what is left of today" should still be
    saying so at three in the afternoon.

    Sticky and high priority by default, which together mean the density budget
    can never collapse one and its time passing never slides it into the struck-
    through past. It leaves when the condition does, and not before.

    Two ways to say when a row applies, both of them Home Assistant's own rather
    than ours:

    * an entity and the states that count, matching any of several because the
      interesting ones rarely have exactly one name — `open, opening` for a
      cover, `unlocked, jammed` for a lock;
    * a template, evaluated by Home Assistant's template engine and tracked by
      its own `async_track_template_result`, for everything a state cannot say.

    Pure, and fed a snapshot rather than `hass`, so every rule here is testable
    without a running Home Assistant. The coordinator resolves templates into
    that snapshot exactly as it resolves entities, which is what keeps the two
    kinds of rule the same shape from here down.
    """
    out: list[Entry] = []
    for index, rule in enumerate(rules):
        ...
        if rule.get("template"):
            # A resolved template is on or off, like any binary thing in Home
            # Assistant. Keeping it in the same vocabulary is what lets one
            # code path serve both kinds of rule.
            if str(current.get("state")) != "on":
                continue
        else:
            wanted = _states(rule)
            if not wanted or str(current.get("state")) not in wanted:
                continue

        # It began when the condition did, so it sorts into the day where it
        # actually happened. Clamped to the day, because something true since
        # Tuesday would otherwise sort above breakfast and read as this
        # morning's news.
        since = _parse(current.get("since")) or day_start
        start = max(since, day_start)
        ...
        entry: Entry = {
            ...
            "kind": "standing",
            "source": "House",
            "title": rule.get("phrase")
            or f"{current.get('name') or key} is {current.get('state')}",
            "automation": None,
            "priority": rule.get("priority") or "high",
            "sticky": True,
            "entity_id": rule.get("entity_id"),
        }

        # The button, when there is one, and always a script.
        #
        # A script is Home Assistant's own name for "a sequence of steps that
        # can branch, check something first, and decline" — which is exactly
        # what a button on a standing row needs to be allowed to do. Calling the
        # underlying entity's service directly would work while skipping the one
        # part that matters. Same rule the `#tag` side already follows: Dayline
        # offers, the script decides, and what it decided is the script's to
        # report by setting something another rule here can watch.
        script = rule.get("script")
        if script:
            entry["action"] = {
                "label": rule.get("button") or "Do it",
...
def _states(rule: dict[str, Any]) -> set[str]:
    """The states a rule matches, however they were written.

    Stored as a list by the options UI, but tolerant of a bare string so a rule
    hand-written in YAML, or one saved before this took several, still works.
    """
    raw = rule.get("state")
